    ...
    \begin{enumerate}[label=\roman*)]
        ...
        original PDE and initial condition. 
            \begin{align*}
                ...
                &= 0 
            \end{align*}
        \item The computed numerical value was 
        $\hat{u}(0.7,0,T) = 0.7833101185224285$ which is
        relatively close to the exact value of $u(0.7,0,T) = 0.782704538242$. 
...
\section{Nonlinear Function of an RV}
    \begin{enumerate}[label=\roman*)]
        \item 
            \begin{align*}
                dY &= \ln\left[X(t) + \sigma X(t)dW(t)\right] - \ln[X(t)]
                \\
                &= \ln[1 + \sigma dW(t)]
            \end{align*}
        \item 
            \begin{align*}
                ...
                &= E(X(t)) + E(dX)
                \\
                &= E(X(t)) + \sigma E(X(t))E(dW(t))
                \\
                E(X(t + dt)) - E(X(t)) &= \sigma E(X(t))E(dW(t))
                \\
                d(E(X(t)) &= \sigma E(X(t))E(dW(t))
                \\
                &= 0
            \end{align*}
            I would like to add a small bit of disccusion here. In the
            derivation for the ODE for $E(Y(t))$, I notice that the ODE obtained
            is directly determined by the order of the expansion for $\ln(1 +
            \sigma dW)$, i.e. if we use a first order expansion we find $E(Y(t))
            = Y(0)$ and if we use a second order expansion we find $E(Y(t)) =
            Y(0) - \frac{\sigma^2}{2}t$. It seems appropriate to use only a
            first order expansion as this only includes changes to $Y(t)$ which
            vary with respect to $dW(t)$ as $X(t)$ does. Thus, we obtain the
            same ODE for $E(Y(t))$ as we do for $E(X(t))$, albeit with different
            initial conditions. 
            \begin{align*}
                E(Y(t + dt)) &= E(Y(t) + dY)
                \\
                &= E(Y(t)) + E(\ln(1 + \sigma dW(t)))
                \\
                dE(Y(t)) &= E(\ln(1 + \sigma dW(t)))
                \\
                &\approx E\left(\ln(1) + \sigma dW(t) 
                + h.o.t.\right)
                \\
                &\approx \sigma E(dW(t))
                \\
